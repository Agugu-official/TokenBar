...
- **Usage attribution now states which routes the engine merged before the page saw them.** [#211](https://github.com/Nanako0129/TokenBar/pull/211)

  The page explained that provider IDs are compared exactly as the source emitted them, so related-looking routes may appear as separate rows. That is true of the page and misleading about what reaches it: the engine folds `vertex` and `vertex_ai` into `anthropic`, and `openai_codex` into `openai`, before any report is built. Vertex-hosted Claude and Anthropic direct are two billing relationships arriving under one key, and no later stage can separate them. The hint now carries both facts instead of implying a separation that does not exist.
