...
> **Windows secure-storage ownership（TB-WIN-165-E1 partial foundation）：** `agent_storage_windows.rs` owns the low-level CNG、exact protected DACL、secure-open、identity、lock、replace與quarantine semantics；account-scope and quota-history consumer workflows remain their own owners and are not duplicated here. The caller must anchor paths beneath a trusted per-user data root. The final path component must not be a reparse point; an ancestor reparse below that trusted anchor is allowed. This low-level claim does not defend against an arbitrary malicious process running under the same SID. Issue #165 remains open until `TB-CRED-164` retires the consumer duplicates; `TB-CRED-164` is the next consumer slice.
