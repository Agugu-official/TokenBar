...
| Usage attribution | 政策以結構性斷言驗證而非逐條列舉：每個訂閱服務的 provider 都必須（a）有明確的 cross-agent 政策、（b）有已知的 opencode 標籤、（c）該標籤解析到確實服務該 provider 的 client、（d）指名 `providerOwnClient`。四項缺一，新增 provider 時就會靜默落入寬鬆分支。另驗證來源 client 可為註冊表外的動態 id 而目標不可、無擁有者的標籤不成為目標、以及 bound provider 的轉售訂閱仍可指派 |
