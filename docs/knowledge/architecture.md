...
| 跨 client 建議的政策 | `crossAgentSubscriptionProviders`（`openai`、`xai`）允許從其他 agent 使用；`subscriptionBoundProviders`（`anthropic`、`google`）不允許。兩者必須互斥，且每個訂閱服務的 provider 都要有明確歸屬 |
...
| opencode 的第二個所有權來源 | `opencodeSubscriptions` 以顯示標籤報告已 oauth 的 provider。四個改名（`Codex`／`Claude`／`Copilot`／`Gemini`）需顯式反查，其餘標籤是大寫的 provider key，透過 `subscriptionProviderMap` 解析；無擁有者的標籤不是目標 |
